    ...
    for symbol in list(strategy['Book']):
        ...
        if not stock_data.empty:
            ...
            if purchase_info:
                ...
                #             hold_period = stock_data['High_Risk_Score_HoldPeriod'].iloc[0]
                ...
                #                     annualized_return = (current_price / purchase_price) ** (365 / days_held) - 1 if days_held > 0 else 0
                #                     if (annualized_return >= annualized_gain or 
                #                         (current_price - purchase_price) / purchase_price <= loss_threshold or 
                #                         days_held >= hold_period):  # Use the extracted hold_period here
                #                         sell_stock(strategy, symbol, current_price, current_date, days_held)
                #                 else:
                #                     annualized_return = (current_price / purchase_price) ** (365 / days_held) - 1 if days_held > 0 else 0
...
# 7.29.24 - moved over here from down below by IMPORTANT
    ...
    st.write(f"Date range: {full_start_date.strftime('%m-%d-%Y')} to {full_end_date.strftime('%m-%d-%Y')} | Number of available symbols:", len(unique_symbols),f"|  Last updated: {file_update_date.strftime('%m-%d-%Y %H:%M')}")
...
# 10.26.24 - MOVED GAUGE FROM HERE TO BELOW TO REFLECT ON NEW MAIN MENU SECTION
    ...
    if True:  # st.button("Generate Market Gauge"):
        ...
        with col2:
            ...
            main_generate_button = st.button("▶️  Run Simulation ", key="main_generate_portfolio", use_container_width=True)
...
# 10.25.24 - version 2
# Main menu with 3 horizontal categories
    tab1, tab2, tab3 = st.tabs(["Zoltar Ranks Index", "Fine-Tune Preferences", "Risk Tolerance"])
    ...
    # Category 3: Risk Tolerance
    with tab3:
        centered_header_main("Risk Tolerance")
    ...
    with st.sidebar:
        ...
        if True: # st.button("Generate Market Gauge"):
            ...
            fig = go.Figure(go.Indicator(
                ...
                title={'text': f"Market Gauge for {next_bd}<br>using {risk_level} Risk Rank {'w/ Sharpe' if use_sharpe else ''}"}
            ))
