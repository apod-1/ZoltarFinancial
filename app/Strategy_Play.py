...
# 10.26.24 - MOVED GAUGE FROM HERE TO BELOW TO REFLECT ON NEW MAIN MENU SECTION
    ...
    with maintab3:
        ...
        with st.expander("Zoltar Rank Version Settings", expanded=False):
            col1set, col2set, col3set = st.columns([1, 1, 1])
            with col1set: 
                num_versions = st.slider("Select number of versions to go back", 1, 200, 50, help="ATTENTION: The web app has a limitation and may crash with large input", key=f"{risk_level}_long_view_research2")
